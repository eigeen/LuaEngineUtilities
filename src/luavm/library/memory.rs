...
impl UserData for Memory {
    fn add_methods<'lua, M: LuaUserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_function("offset", |_, (base, offsets): (usize, Vec<isize>)| {
            utils::get_ptr_with_offset(base as *const u8, &offsets)
...
impl RawPtr {
    ...
    pub fn override_offset(&mut self, offset: isize) {
        if self.offsets.is_empty() {
            self.offsets.push(offset);
        } else {
            self.offsets.insert(self.offsets.len(), offset)
        }
    }
    ...
    where
        T: Copy,
    {
        utils::get_value_with_offset(self.base as *const T, &self.offsets)
    }

    pub fn get_ptr<T>(&self) -> Option<*const T> {
        utils::get_ptr_with_offset(self.base as *const T, &self.offsets)
    }
    ...
    pub fn set_value<T>(&self, value: T) -> Result<(), String> {
        let ptr = utils::get_ptr_with_offset(self.base as *const T, &self.offsets)
